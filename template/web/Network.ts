...
import { Contracts as mainnetV2Roots } from '../compound-config/networks/mainnet.json';
import mainnetV2Abi from '../compound-config/networks/mainnet-abi.json';

import { Contracts as goerliV2Roots } from '../compound-config/networks/goerli.json';
import goerliV2Abi from '../compound-config/networks/goerli-abi.json';
...
export type RootsV3<Network> =
  ...
  Network extends 'goerli' ? typeof mainnetV3Roots :
  never;
...
export function getNetworkById(chainId: number): Network | null {
  ...
  if (chainId === 1) {
    ...
    return 'goerli';
  } else if (chainId === Number(import.meta.env.VITE_chain_id!)) {
    return "mainnet"
  }
   else {
    return null;
...
export function goerliConfig<N extends 'goerli'>(network: N): NetworkConfig<'goerli'> {
  ...
  const extAbi = cometExtAbi;
  const rootsV3: RootsV3<'goerli'> = mainnetV3Roots;
